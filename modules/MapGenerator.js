...
var create = function (size) {
    ...
    var spawn = function (map, spawnSize) {
        ...
        z = spawnSize + 1;
        while (unConnected) {
            if (map[z + spawnSize / 2 * width2] == 25) { unConnected = false; }
        ...
        z = spawnSize + 1;
        while (unConnected) {
            if (map[spawnSize / 2 + z * width2] == 25) { unConnected = false; }
        ...
        while (unConnected) {
            if (map[z + h * width2] == 25) { unConnected = false; }
            map[z + h * width2] = 25;
            map[z + (h + 1) * width2] = 25;
            z--;
        }
        ...
        while (unConnected) {
            if (map[h + z * width2] == 25) { unConnected = false; }
            map[h + z * width2] = 25;
            map[(h+1) + z * width2] = 25;
            z--;
        }
    ...
    if (size == 'small') { height = 16; spawnSize = 2; }
    else if (size == 'medium') { height = 30; spawnSize = 4; }
    else if (size == 'large') { height = 44; spawnSize = 6; }
    else { console.log('entree invalide'); return; }
    ...
    var json = JSON.stringify(map);
    console.log("end");
    var nameFile = './www/assets/random-map-' + size + '.json';
